...
export type RequestPart = () => string;
export type QueryPart = () => { [key: string]: any };

export class FhirRequest<T extends fhir4.Resource> {
	requestType: Request<T>;
	parts: RequestPart[];
	...
	async do(data?: any): Promise<T> {
		const url = new URL(window.location.protocol + window.location.host);

		url.pathname = process.env.REACT_APP_HAPI_BASE_URL;
		...
		if (this.queryParts) {
			const query = this.queryParts.reduce((acc, part) => {
				return { ...acc, ...part() };
			}, {});
		...
		const response = await this.requestType(url, data);
